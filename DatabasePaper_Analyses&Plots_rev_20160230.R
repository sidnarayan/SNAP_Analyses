  ...
  mutate(
    ...
    Habitat.Cost.Reported.Total=ifelse(Cost.Unit=="Total",Reported.Cost.Mean,
                                       ifelse(Cost.Unit=="Ha",Reported.Cost.Mean*(Area/10000),
                                              ifelse(Cost.Unit=="KM",Reported.Cost.Mean*(Length/1000),
                                                     ifelse(Cost.Unit=="M",Reported.Cost.Mean/Width,
                                              Reported.Cost.Mean*Area)))),
    Habitat.Cost.USD2014=Habitat.Cost.Reported.Total*CPI.2014*USD.2014,
    Habitat.Cost.Per.Meter=Habitat.Cost.USD2014 / Length,
    Habitat.Cost.Per.Sq.Meter=Habitat.Cost.USD2014 / Area,
...
suppltable_costs<-projectdata[c("ID","First_Author","Year","Country","Habitat","Latitude","Longitude","Width","Length","Area","Objective","Exposure","Habitat.Cost.Reported.Total","Cost.Year","Currency","CPI.2014","USD.2014","Habitat.Cost.USD2014","Habitat.Cost.Per.Meter","Reported.Benefit.Type")]
...
suppltable_NbDpairs<-nearbyprojects[c("Proj_ID","Exp_ID","Proj_Study","Exp_Study","Habitat","Country","Proj_lat","Proj_lon","Exp_lat","Exp_lon","Proj_width","Exp_width","Exp_depth","Exp_Hs","Proj_Rpercent","Proj_cost_per_m","Proj_Cost_Unit")]
write.table(suppltable_NbDpairs, "C:/Rfiles/SNAP/NbDpairs.txt", sep="\t",row.names=F)
...
signiftest[1:4,1]=c("Coral Reefs (34)","Salt_Marshes (15)","Mangroves (14)","Seagrass/Kelp Beds (6)")
...
effectsizeplot<-ggplot(data=signiftest)+geom_point(aes(x=Habitat,y=Effect_Size),size=4)+theme_bw()+
  ...
  labs(x="Habitat Type (number of studies)",y="Wave Height Reduction Log Response Ratio")
effectsizeplot
...
creationprojectdata<-subset(projectdata,projectdata$Habitat.Cost.Per.Sq.Meter!="NaN")

creationprojectdata<-group_by(creationprojectdata, HabitatName)
meanvals<-creationprojectdata %>%
  mutate(
    MeanCost=mean(Habitat.Cost.Per.Sq.Meter),
    ULMCost=MeanCost+qt(0.975,df=length(Habitat.Cost.Per.Sq.Meter)-1)*sd(Habitat.Cost.Per.Sq.Meter)/sqrt(length(Habitat.Cost.Per.Sq.Meter)),
    LLMCost=MeanCost-qt(0.975,df=length(Habitat.Cost.Per.Sq.Meter)-1)*sd(Habitat.Cost.Per.Sq.Meter)/sqrt(length(Habitat.Cost.Per.Sq.Meter)),
    MedianCost=median(Habitat.Cost.Per.Sq.Meter),
    MinCost=min(Habitat.Cost.Per.Sq.Meter),
    MaxCost=max(Habitat.Cost.Per.Sq.Meter),
    MeanArea=mean(Area),
...
meanvals<-meanvals %>%
  dplyr::select(HabitatName, MeanCost, ULMCost,LLMCost,MedianCost, MinCost,MaxCost, MeanArea, MedianArea, MinArea, MaxArea) %>%
...
colnames(nearbyprojects)<-c("Proj_ID","Exp_ID","Proj_Study","Exp_Study","Habitat","Country","Proj_lat","Proj_lon","Exp_lat","Exp_lon","Proj_area","Proj_width","Exp_width","Exp_depth","Exp_Hs","Exp_Tp","Exp_R","Exp_R_per_m","Exp_r","Proj_Cost_Unit","Proj_cost_per_m","Proj_B_type","Proj_Rpercent")
k=0
for(i in 1:length(projdata$ID)) {
  for (j in 1:length(fielddata$ID)) {
    ...
    if((distance<=50)&&(fielddata$R[j]>0))
    {
      ...
      nearbyprojects$Exp_r[k]<-as.numeric(fielddata$r[j]) # This r = R*Hs.i/B 
      nearbyprojects$Proj_Cost_Unit[k]<-as.character(projdata$Cost.Unit[i])
      nearbyprojects$Proj_cost_per_m[k]<-as.numeric(projdata$Habitat.Cost.Per.Meter[i])
...
nearbyprojects<-nearbyprojects %>%
  mutate(
    NbD_Kt=ifelse((1-Exp_r*Proj_width/Exp_Hs)>0.05,1-Exp_r*Proj_width/Exp_Hs,0.05),
    NbD_RPercent=(1-NbD_Kt^2)*100,
    Proj_Rpercent=ifelse((Exp_R*100*Proj_width/Exp_width)<100,Exp_R*100*Proj_width/Exp_width,100),
    NbD_cost_per_m=Proj_cost_per_m
    )
nearbyprojects<-filter(nearbyprojects, NbD_cost_per_m!="NA")
...
Kt_All<-struct_comparison %>%
  ...
  select(Proj_ID, Country, Habitat, Proj_width, Proj_depth, Hs, Struc_Bc, Proj_cost_per_m,Proj_R_Total, RCratio, Struc_max_effective_unitcost_per_m, Kt_Family) %>%
  arrange(Proj_depth)
...
mg_struc_RC_mean<-mean(Kt_Mangrove$RCratio)
sm_struc_RC_mean<-mean(Kt_SaltMarsh$RCratio)
mg_struc_RC_sd<-sd(Kt_Mangrove$RCratio)
sm_struc_RC_sd<-sd(Kt_SaltMarsh$RCratio)
mg_struc_RC_margin<-qt(0.975,length(Kt_Mangrove)-1)*mg_struc_RC_sd/sqrt(length(Kt_Mangrove))
sm_struc_RC_margin<-qt(0.975,length(Kt_SaltMarsh)-1)*sm_struc_RC_sd/sqrt(length(Kt_SaltMarsh))

struceffcostvals=data.frame(matrix(NA,ncol=4,nrow=2))
colnames(struceffcostvals)<-c("Habitat","Replacement Cost Ratio","95% CI Upper", "95% CI Lower")
struceffcostvals[1:2,1]=c("Mangrove","Salt-Marsh")
struceffcostvals[1:2,2]=c(mg_struc_RC_mean,sm_struc_RC_mean)
struceffcostvals[1:2,3]=c(mg_struc_RC_mean-mg_struc_RC_margin, sm_struc_RC_mean-sm_struc_RC_margin)
struceffcostvals[1:2,4]=c(mg_struc_RC_mean+mg_struc_RC_margin, sm_struc_RC_mean+sm_struc_RC_margin)
struceffcostvals
