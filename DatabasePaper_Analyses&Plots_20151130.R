...
meta_marshes<- smdata %>%
  mutate(
    RR=HsT/HsC,
    Yi=log(RR), #Effect size
    Vyi=(HsCErr^2)/(n*HsC^2)+(HsTErr^2)/(n*HsT^2),
    Wi=1/Vyi,
    Wi2=Wi^2,
    Wi.Yi=Wi*Yi,
    Wi.Yi2=Wi*Yi^2,
...
## Variable Transformations After Excluding All HsC = 1
...
MGhvplot<-ggplot(data=mgdata)+geom_point(aes(x=hv,y=R),shape=0)+
  labs(title="Mangroves",x="Height of vegetation (m)",y="Wave Reduction %")+
  theme_bw()+ geom_smooth(data=mgdata,aes(x=hv,y=R),method=lm,formula=y~x,se=FALSE,linetype=2,colour="black")+
  annotate('text',x=0.9,y=0.23,label=paste("R^2==",format(summary(mg_hv_lm)$adj.r.squared, digits=4)),parse=TRUE)
MGhvplot

# h vs Wave Attn
mg_h_lm<-lm(R~h, mgdata)
MGhplot<-ggplot(data=mgdata)+geom_point(aes(x=h,y=R),shape=0)+
  labs(title="Mangroves",x="Water depth (m)",y="Absolute Wave Attenuation (Hsc - HsT) (m)")+
  theme_bw()+geom_smooth(data=mgdata,aes(x=h,y=R),method=lm,formula=y~x,se=FALSE,linetype=2,colour="black")+
  annotate('text',x=0.9,y=0.32,label=paste("R^2==",format(summary(mg_h_lm)$adj.r.squared, digits=4)),parse=TRUE)
MGhplot
...
SMHsplot<-ggplot(data=smdata)+geom_point(aes(x=HsC,y=R),shape=5)+
  labs(title="Salt-Marsh",x="Incident Wave Height, HsC (m)",y="Wave Reduction %")+
  theme_bw()+scale_y_continuous(labels=function(x)x*100)
SMHsplot
...
sm_hv_lm<-lm(R~hv,smdata)
SMhvplot<-ggplot(data=smdata)+geom_point(aes(x=hv,y=R),shape=5)+
  labs(title="Salt-Marsh",x="Vegetation height (m)",y="Wave Reduction %")+
  theme_bw()+geom_smooth(data=smdata,aes(x=hv,y=R),method=lm,formula=y~x,se=FALSE,linetype=2,colour="black")+
  annotate('text',x=0.6,y=0.35,label=paste("R^2==",format(summary(sm_hv_lm)$adj.r.squared, digits=4)),parse=TRUE)
SMhvplot
...
SGHsplot<-ggplot(data=sgdata)+geom_point(aes(x=HsC,y=R),shape=8)+
  labs(title="Seagrass/Kelp",x="Incident Wave Height, HsC (m)",y="Wave Reduction %")+
  theme_bw()+scale_y_continuous(labels=function(x)x*100)
SGHsplot
